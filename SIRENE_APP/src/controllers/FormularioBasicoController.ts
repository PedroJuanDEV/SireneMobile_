...
import { FormularioBasicoService } from '../services/FormularioBasicoService';

export class FormularioBasicoController {
  static async criarFormulario(req: Request, res: Response): Promise<void> {
    try {
      const formulario = await FormularioBasicoService.criarFormulario(req.body);
      res.status(201).json(formulario);
    } catch (error) {
      res.status(400).json({ error: 'Erro ao criar formulário básico', details: error });
    }
  }

  static async listarFormularios(req: Request, res: Response): Promise<void> {
    try {
      const formularios = await FormularioBasicoService.listarFormularios();
  ...
  static async buscarPorId(req: Request, res: Response): Promise<void> {
    try {
      ...
      const formulario = await FormularioBasicoService.buscarPorId(id);
      if (!formulario) {
  ...
  static async atualizarFormulario(req: Request, res: Response): Promise<void> {
    try {
      ...
      const formulario = await FormularioBasicoService.atualizarFormulario(id, req.body);
      res.json(formulario);
    } catch (error) {
  ...
  static async removerFormulario(req: Request, res: Response): Promise<void> {
    try {
      ...
      await FormularioBasicoService.removerFormulario(id);
      res.status(204).send();
    } catch (error) {
