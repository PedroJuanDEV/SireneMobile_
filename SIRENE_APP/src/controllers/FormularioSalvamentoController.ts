...
import { FormularioSalvamentoService } from '../services/FormularioSalvamentoService';

export class FormularioSalvamentoController {
  static async criarFormulario(req: Request, res: Response): Promise<void> {
    try {
      const formulario = await FormularioSalvamentoService.criarFormulario(req.body);
      res.status(201).json(formulario);
    } catch (error) {
      res.status(400).json({ error: 'Erro ao criar formulário de salvamento', details: error });
    }
  }

  static async listarFormularios(req: Request, res: Response): Promise<void> {
    try {
      const formularios = await FormularioSalvamentoService.listarFormularios();
  ...
  static async buscarPorId(req: Request, res: Response): Promise<void> {
    try {
      ...
      const formulario = await FormularioSalvamentoService.buscarPorId(id);
      if (!formulario) {
  ...
  static async atualizarFormulario(req: Request, res: Response): Promise<void> {
    try {
      ...
      const formulario = await FormularioSalvamentoService.atualizarFormulario(id, req.body);
      res.json(formulario);
    } catch (error) {
  ...
  static async removerFormulario(req: Request, res: Response): Promise<void> {
    try {
      ...
      await FormularioSalvamentoService.removerFormulario(id);
      res.status(204).send();
    } catch (error) {
