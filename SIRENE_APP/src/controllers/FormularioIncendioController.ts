...
import { FormularioIncendioService } from '../services/FormularioIncendioService';

export class FormularioIncendioController {
  static async criarFormulario(req: Request, res: Response): Promise<void> {
    try {
      const formulario = await FormularioIncendioService.criarFormulario(req.body);
      res.status(201).json(formulario);
    } catch (error) {
      res.status(400).json({ error: 'Erro ao criar formulário de incêndio', details: error });
    }
  }

  static async listarFormularios(req: Request, res: Response): Promise<void> {
    try {
      const formularios = await FormularioIncendioService.listarFormularios();
  ...
  static async buscarPorId(req: Request, res: Response): Promise<void> {
    try {
      ...
      const formulario = await FormularioIncendioService.buscarPorId(id);
      if (!formulario) {
  ...
  static async atualizarFormulario(req: Request, res: Response): Promise<void> {
    try {
      ...
      const formulario = await FormularioIncendioService.atualizarFormulario(id, req.body);
      res.json(formulario);
    } catch (error) {
  ...
  static async removerFormulario(req: Request, res: Response): Promise<void> {
    try {
      ...
      await FormularioIncendioService.removerFormulario(id);
      res.status(204).send();
    } catch (error) {
